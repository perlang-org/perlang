## [0.7.0] - 2025-09-17
- The seventh public release of Perlang. For the easiest way to install this on your machine, see
  https://perlang.org/download/. **Note** that only Linux is currently supported. We hope to re-add support for other
  platforms at some point in the future.
- This release focuses on adding features necessary for rewriting the `PerlangScanner` class in Perlang ([#505][505]).
  Generally useful features like `class` support was also added during this release cycle. For the full details, see the
  list below.
...
- Support dynamically allocated `int` arrays [[!629][629]]
- Support dynamically allocated object arrays [[!630][630]]

### `stdlib`
- Fix `printf`-format-related warning [[!574][574]]
- Add more `StringBuilder::append` and `append_line` overloads: 6d819f2
...
### Maintenance
 ...
 - Minor `IntArray` cleanups: 734cde7

### CI
- Make condition for building releases stricter: 65f58ca
- Remove all other snapshot platforms than `linux-x64`: de8dbe3
- Run all GitHub Actions on `ubuntu-22.04` image: 45f3d7a
- Remove `docs-validate-api-docs` check: 630615c
- Run `stdlib`/`perlang_cli` compilation on all available CPUs [[!622][622]]
- Workaround for GitLab Release file name limitation: 1e70eb0

### Tests
 - In this release, the total number of tests is 2030 (counted by the `test: []` CI job).
 - Improve `AssertFail*` exception handling: [[!600][600]]
 - More `AssertFail*` exception message improvements [[!601][601]]
 - Use explicit `private` visibility for test methods: 7420119
...
- Adjust available platforms on the Download page (to clarify that only Linux binaries are currently available): 55843a9
- Remove API docs: 4acaf4a
...
[629]: https://gitlab.perlang.org/perlang/perlang/merge_requests/629
[630]: https://gitlab.perlang.org/perlang/perlang/merge_requests/630
