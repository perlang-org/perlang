...
- First release to include `perlang_cli`, which contains the native (Perlang/C++) reimplementation of the Perlang CLI.
  Note that only a very limited part of the CLI is implemented in Perlang yet; the rest is still written in C#. This
  change also means that we unfortunately drop support for Windows and macOS for now. See [#511][issue-511] and
  [#512][issue-512] for issues tracking macOS and Windows support respectively.
...
- scripts: Fix `github.com` references in `perlang-install`: 3536a84
- scripts: Improve *BSD error messages in `perlang-install`: 91fa8bb
...
[issue-511]: https://gitlab.perlang.org/perlang/perlang/-/issues/511
[issue-512]: https://gitlab.perlang.org/perlang/perlang/-/issues/512
