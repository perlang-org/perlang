...
// Force StyleCop to ignore the file
// <auto-generated/>
...
namespace Mono.Terminal {
	...
	public class LineEditor {
		...
		// The number of milliseconds to wait between each attempt to read keypresses
		int read_delay = 10;

		// If we are done editing, this breaks the interactive loop
		bool done = false;

		// Set to true on Ctrl-C to break the interactive loop
		...
		public LineEditor (string name, int histsize)
		{
			handlers = new Handler [] {
				...
				Handler.Control ('H', CmdBackspace),
				Handler.Control ('J', CmdDone, resetCompletion: false),
				...
				Handler.Control ('U', CmdCutToBeginningOfLine ),
		...
		void Complete ()
		{
			...
			mismatch:
				...
				if (last != -1){
					// QUIRK(perlun): Disabled since it causes weird behavior for me
		...
		void CmdCutToBeginningOfLine()
		{
			kill_buffer = text.ToString (0, cursor);
		...
		void EditLoop ()
		{
			...
			while (!done && !interrupted){
				...
				cki = ReadKeyHandlingSigint (read_delay);
		...
		ConsoleKeyInfo ReadKeyHandlingSigint (int delay)
		{
			while (!interrupted)
			{
		...
		public string Edit (string prompt, string initial)
		{
			...
			do {
				try {
					...
					// Make it clear to the user that Ctrl-C was pressed
					Console.Write ("^C");
